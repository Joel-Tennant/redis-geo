...
echo is this the problem line

eval $(aws ecr get-login --region $AWS_DEFAULT_REGION --no-include-email | sed 's|https://||' | yes) | #needs AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY envvars
echo yes
